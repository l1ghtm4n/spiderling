...
class MimeTypeHelper {
	...
	const AUTO_DOWNLOAD = [
		...
		'doc', 'docx','docm',
		'xls', 'xlsx',
		'ppt', 'pptx','pot','pps','pptm',
		'odm','odt','ott','odp'
		
	];

	const ARCHIVE_FILE = ['zip', 'rar', 'arc', 'bz', 'bz2', 'tar', '7z']; //Cần bổ sung thêm

	public static function guess_extension( $path , $default = '') {
		$mimetype = mime_content_type( $path );
		$ext = array_get(self::MIMTYPES, $mimetype, '');
		if(empty($ext)){
			return $default;
		}
		return $ext;
	...
	public static function is_auto_download($type, $isExt = false){
	    if (!$isExt){
            $ext = array_get(self::MIMTYPES, $type, '');
            if(empty($ext)){
                return false;
            }
        }else{
		...
		if(!in_array($ext, self::AUTO_DOWNLOAD)){
			return false;
		}
		return $ext;
	}
	 ...
	 * @param $header
	 *
	 * @return bool|mixed trả về ext nếu có auto download
	 */
	public static function is_auto_download_header($header){
		$ct = array_get($header, 'Content-Type', false);
		if(!$ct){
			$ct = array_get($header, 'content-type', false);
		}
		if(!$ct){
            $l = count($header) - 1;
			$ct = array_get($header, $l.'.Content-Type', false);
            if(!$ct){
                $ct = array_get($header, $l.'.Content-type', false);
            }
            if(!$ct){
		...
		$ct = preg_replace('/\;.*$/', '', $ct);
		return self::is_auto_download($ct);
	}
    public static function ext_from_mime($mime, $default = false){
	    $ext = $ext = array_get(self::MIMTYPES, $mime, '');
	    if(empty($ext)){
	        return $default;
        }else{
	        return $ext;
    ...
    public static function isArchive($mimetype, $isExt = false){
        if ($isExt){
            $ext = $mimetype;
        }else{
            $ext = $ext = array_get(self::MIMTYPES, $mimetype, '');
            if(empty($ext)){
                return false;
            }
        }

        if(!in_array($ext, self::ARCHIVE_FILE)){
            return false;
        }
        return $ext;
    }
